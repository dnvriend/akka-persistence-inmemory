...
class EventsByPersistenceIdPublisher(persistenceId: String) extends ActorPublisher[EventEnvelope]
  ...
  def init: Receive = {
    case _: Request ⇒
      journal ! InMemoryJournal.EventsByPersistenceIdRequest(persistenceId)
