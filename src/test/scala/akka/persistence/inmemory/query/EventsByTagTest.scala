...
class EventsByTagTest extends QueryTestSpec {
  ...
  it should "find events for one tag, starting with non-empty journal" in {
    ...
    withEventsByTag()("number", 0) { tp =>
      ...
      tp.expectNext(EventEnvelope(4, "my-1", 2, "a-2"))

      persist(3, 3, "my-1", "number") // 5
      tp.expectNext(EventEnvelope(5, "my-1", 3, "a-3"))

      persist(4, 4, "my-1", "number") // 6
      tp.expectNext(EventEnvelope(6, "my-1", 4, "a-4"))

      persist(2, 2, "my-2", "number") // 7
      tp.expectNext(EventEnvelope(7, "my-2", 2, "a-2"))

      persist(2, 2, "my-3", "number") // 8
      tp.expectNext(EventEnvelope(8, "my-3", 2, "a-2"))
